import * as THREE from './resource/js/three/three'
//import { FirstPersonControls } from './resource/js/three/firstPersonControl'
...
function init() {
  ...
  /*renderer*/
  renderer = new THREE.WebGLRenderer({ context: canvas.getContext("webgl") });
  ...
  camera.position.set(-250, 40, 10);
  camera.lookAt(new THREE.Vector3(0, -9, -1));
  scene.add(camera);
  ...
  // camera.lookAt(new THREE.Vector3(-2, -30, -2));
  renderer.render(scene, camera);
...
function moveEachStar(star) {
  ...
  if (star.angle > Math.PI * star.distance && star.angle) {
